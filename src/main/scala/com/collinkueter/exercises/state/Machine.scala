...
import com.collinkueter.exercises.state.Machine._
import com.collinkueter.exercises.state.State._
...
object Machine {
  ...
  def simulateMachine(inputs: List[Input]): State[Machine, (Coins, Candies)] = {
    val states: List[State[Machine, Unit]] = inputs.map { i =>
      modify[Machine] { m =>
        (i, m) match {
          case (Coin, Machine(true, candies, coins)) if candies > 0 => Machine(locked = false, candies, tagCoins(coins + 1))
          case (Turn, Machine(false, candies, coins))               => Machine(locked = true, tagCandies(candies - 1), coins)
          case _                                                    => m
        }
      }
    }
